		...
		<div class="row mt-3">
			...
			<table class="table table-bordered table-striped ">
				...
				<tbody>
                    ...
                    $result = $conn->query("SELECT `acts_id`,`efficiency`, `timeliness`, `quality`, `accuracy` FROM `evaluations` WHERE acts_id ");
                    while($row = $result->fetch_assoc()) {

						$er=$row['acts_id'];
						$ad=$conn->query("SELECT `name` FROM `activities` WHERE id=$er");
